...
int getEdad(char[],int,int);
int getDni(char menssage[]);
int GetTheName(char[],char[]);
int GetTheLastName(char[],char[]);

int validaFacorialTable(float);
int validaFacorial(float);
int validoOpcion(int ,int,int);
...
int getAlpha(char*,char[],char[],char[],int,int);
int GetAlphaName(char[],char[],int);
void warningMessageWtime(char[],int);
void errorMssgeAndTime(char[],int);
int getStringAlpha(char[],char[]);
